...
class Sjf:
    ...
    def add_processo(self, procc):
        ...
        procc.ID = novo_id
        if self.procc_atual != None and procc.CPUtotal < self.procc_atual.CPUtotal:
            self.fila.append(self.procc_atual)
            self.procc_atual = procc
    ...
    def esc_sjf(self):
        if len(self.fila) > 0:
            fila_ordenada = sorted(self.fila, key=lambda processo: processo.CPUtotal)
